...
import { Folder, Plus, X, Inbox, LayoutGrid } from 'lucide-react';
    ...
    return (
        <aside style={{ width: '280px', background: '#fff', borderRight: '1px solid #eee', padding: '25px 15px' }}>
            ...
            <div style={{ marginBottom: '25px' }}>
                ...
                <button onClick={() => onFolderSelect('unorganized')} style={btnStyle(selectedFolderId === 'unorganized')}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', padding: '0 10px' }}>
                <h3 style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#bbb', textTransform: 'uppercase' }}>Folders</h3>
            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                {folders.map(folder => (
                    <button key={folder.id} onClick={() => onFolderSelect(folder.id)} style={btnStyle(selectedFolderId === folder.id)}>
